...
class Player(Fish):
    ...
    def __init__(self):
        super().__init__()
        self.width, self.height, self.x, self.y = Player.initial_pw, Player.initial_ph, 455, 250
    ...
    def _move_right(self, _) -> None:
        if self.x > 955 or GameControl.stop:
            return
        if not self.anchor:
            self.reversal()
        else:
...
class RandomFish(Fish):
    ...
    def __init__(self):
        ...
        # 0 往左游; 1 往右游
        self.direction = 1 if self.x < 0 else 0
    ...
    def random_type(self, size_id: int) -> int:
        ...
        file_name = f"Resources/{dir_name}/{self.proportion}_{self.direction}.png"
    ...
    @classmethod
    def random_x(cls) -> int:
        # 这即是x坐标也是鱼出现时的方向，一值两用!
        return choice(RandomFish.X)
    ...
    def random_height(cls, rough_size: int) -> int:
        match rough_size:
            case 0:
                ...
                return randint(10, small_upper_size)
            case 1:
                ...
                return randint(medium_lower_size, medium_upper_size)
            case 2:
                ...
                return randint(huge_lower_size, huge_upper_size)

    def random_width(self, rough_size):
        target_folder = f"resources/{RandomFish.resource_dir[rough_size]}"
    ...
    def eat_fish(self):
        without_touching = (
                (RandomFish.basic_player.x + RandomFish.basic_player.width < self.x)
                or (self.x + self.width < RandomFish.basic_player.x)
                or (RandomFish.basic_player.y + RandomFish.basic_player.height < self.y)
                or ((self.y + self.height) < RandomFish.basic_player.y)
        )
        if without_touching:
            return
