...
class MainGUI(Tk):
    ...
    def set_window(self):
        ...
        geometry = f'{env.WIDTH}x{env.HEIGHT+env.MENU_BAR_HEIGHT}+{(screenwidth-env.WIDTH)//2}+{(screenheight-env.HEIGHT)//2}'
        self.geometry(geometry)
        self.resizable(False, False)
        self.title('大鱼吃小鱼')

    def set_background(self) -> ImageTk.PhotoImage:
        bg_img: Image.Image = Image.open(env.BG_IMG_PATH)
        bg_img = bg_img.resize((env.WIDTH, env.HEIGHT))
    ...
    def set_basic_frame(self) -> Canvas:
        ...
        basic_frame.place(y=env.MENU_BAR_HEIGHT, relwidth=1, relheight=1-env.MENU_BAR_HEIGHT/env.HEIGHT+0.01)
        return basic_frame

    def set_menu_bar(self) -> Frame:
        menu_bar = Frame(self, bg='#2AD95D')
        menu_bar.place(height=env.MENU_BAR_HEIGHT, x=0, relwidth=1)
        return menu_bar

    def set_reset_btn(self) -> Button:
        reset_btn = Button(self.menu_bar, bd=0, bg='#FFA500', text='重置', font=(env.FONT, 16))
        reset_btn.place(relwidth=0.1, relheight=0.75, y=5, relx=0.01)
        return reset_btn

    def set_setting_btn(self) -> Button:
        setting_btn = Button(self.menu_bar, bd=0, bg="#FFA500", text='设置', font=(env.FONT, 16))
        setting_btn.place(relwidth=0.1, relheight=0.75, y=5, relx=0.12)
        return setting_btn

    def set_ranking_list_btn(self) -> Button:
        ranking_list_btn = Button(self.menu_bar, bd=0, bg='#FFA500', text='排行榜', font=(env.FONT, 16))
        ranking_list_btn.place(relwidth=0.1, relheight=0.75, y=5, relx=0.23)
    ...
    def set_score_label(self) -> Label:
        ...
        score_label.place(relwidth=0.25, relheight=0.75, y=5, relx=0.38)
        return score_label

    def set_clock(self) -> Label:
        clock = Label(self.menu_bar, bd=0, bg="#2AD95D", text="用时: 0", font=(env.FONT, 20))
        clock.place(relwidth=0.38, relheight=0.75, y=5, relx=0.5)
        return clock
    ...
    def set_life_bar(self) -> list[Label]:
        ...
        for i in range(5):
            ...
            life_bar.place(width=30, relheight=0.88, y=0, relx=0.95 - 0.04 * i)
    ...
    def set_vectory_window(self) -> tuple[Label, Entry, Button]:
        ...
        win_tip.place(y=40, height=160, relwidth=1)
        self.basic_frame.create_text(
            self.basic_frame.winfo_width() // 2, self.basic_frame.winfo_height() // 2,
            text="为你的胜利记录取个名吧!", font=(env.FONT, 25)
        )
        vectory_record_entry = Entry(self.basic_frame, font=('华文楷体', 20))
        vectory_record_entry.place(relx=0.3, rely=0.56, relwidth=0.42, height=45)
        ensure_btn = Button(self.basic_frame, text="确认", bg="#22B14C", bd=0, font=(env.FONT, 15))
        ensure_btn.place(relx=0.72, rely=0.56, height=45)
    ...
    def set_fail_window(self) -> tuple[Label, Button]:
        ...
        fail_tip.place(rely=0.08, height=160, relwidth=1)
        self.basic_frame.create_text(
            self.basic_frame.winfo_width() // 2, self.basic_frame.winfo_height() // 2,
            text=f"想要获胜还需努力!", font=(env.FONT, 40)
        )
        return fail_tip
